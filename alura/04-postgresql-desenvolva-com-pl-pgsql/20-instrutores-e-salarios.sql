...
CREATE OR REPLACE FUNCTION cria_instrutor(nome_instrutor VARCHAR, salario_instrutor DECIMAL) RETURNS void AS $$
	...
	BEGIN
		...
		-- Média salarial não levando em conta o instrutor que foi inserido
...
SELECT * FROM cria_instrutor('Fulano de tal', 1000);
SELECT * FROM cria_instrutor('Outro instrutor', 400);
